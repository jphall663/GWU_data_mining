...
proc sql;
	...
	select id, category, avg(purchasequantity) as avg_category_quantity, 
		avg(purchaseamount) as avg_category_amount
	from transactions
...
data joined_train; 
	...
	by id category; 
	if avg_category_amount = . then avg_category_amount = 0;
	if avg_category_quantity = . then avg_category_quantity = 0;
	if x;
run;
...
data joined_train_subset; 
	set joined_train; 
	where id < 107014607 and avg_category_quantity > 0 and avg_category_amount > 0; 
run;
